...
## ほかの機能
実はGithubのRelease/latestやTagsで公開されているプラグインであれば~~何でも~~ものによってはいけます(下記参照)
...
**"format":   "direct"**
- ダウンロードするファイルのフォーマットの指定です
- 必須ではありません
    - (未記載)  : デフォルト動作、zipだと思ってダウンロード後展開します
    - direct    : .aufなどを直接ダウンロードする際に使います
        - 圧縮かけずにダウンロードさせるプラグイン向け
        - このモードではplugin1つあたりファイルは1つのみダウンロードできます
        - "copy_file"の配列の先頭にあるファイル名でtempフォルダ内に保存します
        - tempフォルダ内に保存した後"copy_folder"にコピーする流れは通常と同じです
